...
#include "SD_MMC.h"            // **ADDED - SD Card ESP32***
...
#include <SPIFFS.h> // File sytem SPIFFS
...
void setup() {
 ...
 Serial.println("High capacity SD card");

} else {
  Serial.println("Assume MMC, other");
  ...
  //create SPIFFS tempimage
  //File tempIMG = SPIFFS.open(path.c_str(), FILE_READ);
 SD_Card_Mounting(); // See src/extras/SDHelper.h
  ...
  Serial.println("Preparing SD file attachments...");
    ...
    message.addRecipient("Me", "forestbuddy23@gmail.com");
...
//if(!SD_MMC.begin()){
// Serial.printf("Failed to mount again");
//  return;  
//}
    ...
    att.file.storage_type = esp_mail_file_storage_type_sd; //esp_mail_file_storage_type_flash;
